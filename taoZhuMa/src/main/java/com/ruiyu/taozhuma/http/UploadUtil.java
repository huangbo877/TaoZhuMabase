...
//		Log.i(TAG, "请求的fileKey=" + fileKey);
	...
	public void uploadFile(final Map<String, FileItem> fileParams,
			final String fileKey, final String RequestURL,
			final Map<String, String> param) {

		Log.i(TAG, "请求的URL=" + RequestURL);
		Log.i(TAG, "请求的文件数=" + fileParams.size());
		Log.i(TAG, "请求的fileKey=" + fileKey);
		new Thread(new Runnable() { // 开启线程上传文件

					public void run() {
						toUploadFile(fileParams, fileKey, RequestURL, param);
					}
				}).start();
	...
	private void toUploadFile(Map<String, FileItem> fileParams, String fileKey,
		...
		try {
			...
			for (Entry<String, FileItem> fileEntry : fileEntrySet) {
				...
				byte[] fileBytes = getFileEntry(fileEntry.getKey(),
						fileItem.getFileName(), fileItem.getMimeType(), CHARSET);
				dos.write(entryBoundaryBytes);
				dos.write(fileBytes);
				/** 上传文件 */
