 ...
 * 
 * */
public class CopyOfTzmCartFatherListAdapter extends BaseAdapter {
	...
	public CopyOfTzmCartFatherListAdapter(Context c, List<TzmMyCartModel> list,
	...
	public View getView(final int position, View view, ViewGroup viewGroup) {
		...
		if (view == null) {
			...
			viewHolder.v_above = (View) view
					.findViewById(R.id.v_above);
		...
		OnClickListener clickListener = new OnClickListener() {
			...
			public void onClick(View v) {
				...
				case R.id.tv_shopname:
					...
					intent.putExtra(
							"activityId",
							(list.get(Integer.parseInt(v.getTag().toString())).shopId).toString());
				...
				case R.id.im_add:
					if (list.get(position).type == 1
						...
						// 普通商品
						list.get(position).number = list.get(position).number + 1;
						viewHolder.et_num.setText(list.get(position).number
								+ "");
						EventBus.getDefault().post(
								new CartFatherEven(index, list));
						EventBus.getDefault()
								.post(new CartEven(list.get(position).cartType,
										list));
					}
		...
		if (info.tag == 0) {
			...
			if (info.status == 0 || info.isActive == 2) {
				...
				viewHolder.tv_product_name.setTextColor(c.getResources().getColor(R.color.tzm_text_sys_default_light));
				viewHolder.tv_price.setTextColor(c.getResources().getColor(R.color.tzm_text_sys_default_light));
			} else {
				...
				viewHolder.tv_product_name.setTextColor(c.getResources().getColor(R.color.tzm_black));
				viewHolder.tv_price.setTextColor(c.getResources().getColor(R.color.tzm_black));
			}
			// 限量购标志
			if (info.type == 1) {
