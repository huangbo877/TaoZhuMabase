...
public class FlexibleRatingBar extends RatingBar {
	...
	private void init() {
		...
		paintOutline.setStrokeJoin(Paint.Join.ROUND); // Remove this line to
														// create pointy stars
	...
	protected synchronized void onMeasure(int widthMeasureSpec,
		...
		// If starSize matches getHeight, the tips of the star can get cut off
		// due to strokeWidth being added to the polygon size.
		// Make it a bit smaller to avoid this. Also decrease star size and
		// spread them out rather than cutting them off if the
		// height is insufficient for the width.
		starSize = Math.min(height, width / getNumStars());
		if (strokeWidth < 0)
			strokeWidth = (int) (starSize / 15);
	...
	// Create a star polygon with any number of vertices, down to 2, which
	// creates a diamond
	...
	private Path createStarBySize(float size, int steps) {
		...
		float radius = halfSize / interiorAngleModifier; // Adjusts "pointiness"
															// of stars
		...
		for (double step = 0; step < max; step += degreesPerStep) {
			...
			path.lineTo(
					(float) (halfSize - radius
							* Math.sin(step + halfDegreesPerStep)),
					(float) (halfSize - radius
							* Math.cos(step + halfDegreesPerStep)));
		}
		path.close();
		return path;
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		// Default RatingBar changes color when pressed. This replicates the
		// effect.
		BitmapShader shaderFill = updateShader(colorFillOn, colorFillOff);
		BitmapShader shaderFillPressed = updateShader(colorFillPressedOn,
				colorFillPressedOff);
		...
		// Rotate star if desired, and resize to fit the available area. Height
		// and width may change during rotation.
		// Other shapes may not be in desirable orientations, but you can rotate
		// them with setPolygonRotation
		if (polygonRotation != 0) {
			path.computeBounds(rectangle, true);
			float maxDimension = Math
					.max(rectangle.height(), rectangle.width());
			matrix.setScale(starSize / (1.15F * maxDimension), starSize
					/ (1.15F * maxDimension));
		...
		for (int i = 0; i < getNumStars(); ++i) {
			// Default RatingBar only shows fractions in the interior, not the
			// outline.
			paintOutline.setColor(isPressed() ? colorOutlinePressed
					: i < getRating() ? colorOutlineOn : colorOutlineOff);

			path.computeBounds(rectangle, true);
			path.offset(
					(i + .5F) * getWidth() / getNumStars()
							- rectangle.centerX(),
	...
	private BitmapShader updateShader(int colorOn, int colorOff) {
		...
		// Bitmap of width 0 will cause a crash. Make sure it's a positive
		// number.
		int ratingWidth = (int) (getRating() * getWidth() / getNumStars());

		if (ratingWidth <= 0 || getWidth() - ratingWidth <= 0) {
			colorsJoined = Bitmap.createBitmap(getWidth(), getHeight(),
					Bitmap.Config.ARGB_8888);
			colorsJoined.eraseColor(ratingWidth <= 0 ? colorOff : colorOn);
		} else {
			Bitmap colorLeft = Bitmap.createBitmap(ratingWidth, getHeight(),
					Bitmap.Config.ARGB_8888);
			colorLeft.eraseColor(colorOn);
			Bitmap colorRight = Bitmap.createBitmap(getWidth() - ratingWidth,
					getHeight(), Bitmap.Config.ARGB_8888);
			colorRight.eraseColor(colorOff);
			colorsJoined = combineBitmaps(colorLeft, colorRight);
		}
		return new BitmapShader(colorsJoined, Shader.TileMode.CLAMP,
	...
	private Bitmap combineBitmaps(Bitmap leftBitmap, Bitmap rightBitmap) {
		colorsJoined = Bitmap.createBitmap(
				leftBitmap.getWidth() + rightBitmap.getWidth(),
	...
	private void getXmlAttrs(Context context, AttributeSet attrs) {
		TypedArray a = context.getTheme().obtainStyledAttributes(attrs,
				R.styleable.FlexibleRatingBar, 0, 0);
		try {
			colorOutlineOn = a.getInteger(
					R.styleable.FlexibleRatingBar_colorOutlineOn,
					Color.rgb(0x11, 0x11, 0x11));
			colorOutlineOff = a.getInteger(
					R.styleable.FlexibleRatingBar_colorOutlineOff,
					Color.rgb(0x61, 0x61, 0x61));
			colorOutlinePressed = a.getInteger(
					R.styleable.FlexibleRatingBar_colorOutlinePressed,
					Color.rgb(0xFF, 0xB7, 0x4D));
			colorFillOn = a.getInteger(
					R.styleable.FlexibleRatingBar_colorFillOn,
					Color.rgb(0xFF, 0x98, 0x00));
			colorFillOff = a.getInteger(
					R.styleable.FlexibleRatingBar_colorFillOff,
					Color.TRANSPARENT);
			colorFillPressedOn = a.getInteger(
					R.styleable.FlexibleRatingBar_colorFillPressedOn,
					Color.rgb(0xFF, 0xB7, 0x4D));
			colorFillPressedOff = a.getInteger(
					R.styleable.FlexibleRatingBar_colorFillPressedOff,
					Color.TRANSPARENT);
			polygonVertices = a.getInteger(
					R.styleable.FlexibleRatingBar_polygonVertices, 5);
			polygonRotation = a.getInteger(
					R.styleable.FlexibleRatingBar_polygonRotation, 0);
			strokeWidth = (int) a.getDimension(
					R.styleable.FlexibleRatingBar_strokeWidth22, -1);
		} finally {
			a.recycle();
		}
	}
